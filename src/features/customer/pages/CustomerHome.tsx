    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground">
      ...
      <footer className="sticky bottom-0 border-t border-border bg-card">
        <nav className="flex justify-around px-4 pt-2 pb-4">
          <button onClick={() => setActiveSection('home')} className={`flex flex-col items-center gap-1 ${activeSection === 'home' || activeSection === 'confirm-booking' ? 'text-foreground' : 'text-muted-foreground'}`}><span className="material-symbols-outlined">home</span><span className="text-xs">Home</span></button>
          <button onClick={() => setActiveSection('bookings')} className={`flex flex-col items-center gap-1 ${activeSection === 'bookings' ? 'text-foreground' : 'text-muted-foreground'}`}><span className="material-symbols-outlined">calendar_month</span><span className="text-xs">Bookings</span></button>
          <button onClick={() => setActiveSection('profile')} className={`flex flex-col items-center gap-1 ${activeSection === 'profile' ? 'text-foreground' : 'text-muted-foreground'}`}><span className="material-symbols-outlined">person</span><span className="text-xs">Profile</span></button>
        </nav>
      </footer>
    </div>
  );
